...
describe('Bulk Lookup API', () => {
  ...
  it('routes URLs to /api/lookup/url', async () => {
    ...
    expect((urlLookup as ReturnType<typeof vi.fn>).mock.calls.length).toBe(1);
  });
  ...
  it('routes DOIs to /api/lookup/doi', async () => {
    ...
    const data = await response.json();
    expect(data.results[0].success).toBe(true);
    expect((doiLookup as ReturnType<typeof vi.fn>).mock.calls.length).toBe(1);
  });
  ...
  it('routes ISBNs to /api/lookup/isbn', async () => {
    ...
    const data = await response.json();
    expect(data.results[0].success).toBe(true);
    expect((isbnLookup as ReturnType<typeof vi.fn>).mock.calls.length).toBe(1);
  });
  ...
  it('returns summary counts', async () => {
    (urlLookup as ReturnType<typeof vi.fn>)
      .mockResolvedValueOnce({ ok: true, json: async () => ({ data: { title: 'A' } }) });
    (doiLookup as ReturnType<typeof vi.fn>)
