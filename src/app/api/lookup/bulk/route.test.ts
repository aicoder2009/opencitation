...
describe('Bulk Lookup API', () => {
  ...
  it('routes URLs to /api/lookup/url', async () => {
    ...
    expect(lookupUrl).toHaveBeenCalledTimes(1);
  });
  ...
  it('routes DOIs to /api/lookup/doi', async () => {
    ...
    const data = await response.json();
    expect(data.results[0].success).toBe(true);
    expect(lookupDoi).toHaveBeenCalledTimes(1);
  });
  ...
  it('returns summary counts', async () => {
    (lookupUrl as ReturnType<typeof vi.fn>).mockResolvedValueOnce({ ok: true, json: async () => ({ data: { title: 'A' } }) });
    (lookupDoi as ReturnType<typeof vi.fn>).mockResolvedValueOnce({ ok: false, json: async () => ({ error: 'fail' }) });
  ...
  it('handles mixed item types in one batch', async () => {
    (lookupUrl as ReturnType<typeof vi.fn>).mockResolvedValueOnce({ ok: true, json: async () => ({ data: { title: 'URL result' } }) });
    (lookupDoi as ReturnType<typeof vi.fn>).mockResolvedValueOnce({ ok: true, json: async () => ({ data: { title: 'DOI result' } }) });
    const response = await POST(
