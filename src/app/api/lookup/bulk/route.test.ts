...
// Mock the individual route handlers
vi.mock('../url/route', () => ({ POST: vi.fn() }));
vi.mock('../doi/route', () => ({ POST: vi.fn() }));
vi.mock('../isbn/route', () => ({ POST: vi.fn() }));

import { POST as mockUrlHandler } from '../url/route';
import { POST as mockDoiHandler } from '../doi/route';
import { POST as mockIsbnHandler } from '../isbn/route';
...
describe('Bulk Lookup API', () => {
  ...
  it('routes URLs to /api/lookup/url', async () => {
    (mockUrlHandler as ReturnType<typeof vi.fn>).mockResolvedValueOnce(
    ...
    expect(mockUrlHandler).toHaveBeenCalled();
    const req = (mockUrlHandler as ReturnType<typeof vi.fn>).mock.calls[0][0] as NextRequest;
    expect(req.url).toContain('/api/lookup/url');
  });

  it('routes DOIs to /api/lookup/doi', async () => {
    (mockDoiHandler as ReturnType<typeof vi.fn>).mockResolvedValueOnce(
      NextResponse.json({ data: { title: 'Research Article' } }, { status: 200 })
    );
    const response = await POST(makeRequest({ items: ['10.1000/xyz123'] }));
    const data = await response.json();
    expect(data.results[0].success).toBe(true);
    expect(mockDoiHandler).toHaveBeenCalled();
    const req = (mockDoiHandler as ReturnType<typeof vi.fn>).mock.calls[0][0] as NextRequest;
    expect(req.url).toContain('/api/lookup/doi');
  });

  it('routes ISBNs to /api/lookup/isbn', async () => {
    (mockIsbnHandler as ReturnType<typeof vi.fn>).mockResolvedValueOnce(
      NextResponse.json({ data: { title: 'Book Title' } }, { status: 200 })
    );
    const response = await POST(makeRequest({ items: ['9780316769174'] }));
    const data = await response.json();
    expect(data.results[0].success).toBe(true);
    expect(mockIsbnHandler).toHaveBeenCalled();
    const req = (mockIsbnHandler as ReturnType<typeof vi.fn>).mock.calls[0][0] as NextRequest;
