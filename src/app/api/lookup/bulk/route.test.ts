...
describe('Bulk Lookup API', () => {
  ...
  it('routes URLs to /api/lookup/url', async () => {
    ...
    const [req] = (urlPost as ReturnType<typeof vi.fn>).mock.calls[0] as [NextRequest];
  ...
  it('routes DOIs to /api/lookup/doi', async () => {
    ...
    const data = await response.json();
    expect(data.results[0].success).toBe(true);
    const [req] = (doiPost as ReturnType<typeof vi.fn>).mock.calls[0] as [NextRequest];
  ...
  it('routes ISBNs to /api/lookup/isbn', async () => {
    ...
    const data = await response.json();
    expect(data.results[0].success).toBe(true);
    const [req] = (isbnPost as ReturnType<typeof vi.fn>).mock.calls[0] as [NextRequest];
