...
export async function POST(request: NextRequest) {
  try {
    ...
    // Uses direct handler invocation instead of loopback fetch to prevent Host-header SSRF
    const lookupPromises = items.map(async (item) => {
      ...
      try {
        ...
        // Make the API call directly via the handler to prevent SSRF
        const syntheticRequest = new NextRequest(new URL(`http://localhost${apiEndpoint}`), {
          method: "POST",
