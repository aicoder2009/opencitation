...
export default function ProjectDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchProjectAndLists = useCallback(async () => {
    try {
      ...
      // Additionally, we derive the project's lists in-memory from allLists to avoid
      // a redundant backend query.
      ...
      if (allListsResult.success) {
        ...
        setLists(listsData.filter((list: List) => list.projectId === projectId));
      }
    } catch (err) {
