...
## 2024-05-19 - [Optimize PWA Offline Sync Queue]
**Learning:** The PWA sync manager previously processed the offline sync queue sequentially using a standard loop, which caused artificial latency and delayed updates when the application came back online.
**Action:** Implemented a staged parallel approach. By grouping the queue into stages by entity type (projects -> lists -> citations) to preserve referential integrity, and then processing unique entity IDs within each stage concurrently using `Promise.all()`, we eliminate unnecessary waterfall latency.
